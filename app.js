...
function loadPage() {
	setTimeout(() => {
		filter = document.getElementById("body-filter");
...
function submitForm(event) {
	...
	if (!validPasswordChars.test(userPassword)) {
		userPassword = "";
		passwordField.style.animation = "incorrectPasswordShake 0.3s 1";
		passwordRequirements.style.animation = "incorrectPasswordShake 0.3s 1";
		setTimeout(() => {
			passwordField.style.animation = "";
			passwordRequirements.style.animation = "";
		}, 350);
	} else {
